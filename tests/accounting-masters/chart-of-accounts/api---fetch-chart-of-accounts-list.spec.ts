...
test.describe('Accounting Masters @Saovr7ti4', () => {
  test('API - Fetch Chart of Accounts List @Ti5vt0ytc', async ({ page }) => {
    ...
    // STEP 3: Retrieve auth token for API call
    ...
    if (!token) {
      test.info().annotations.push({
        type: 'note',
        description: 'Auth token not found in localStorage; relying on cookies for authentication',
      });
    }

    // STEP 4: Send GET request to accounts API endpoint with valid auth headers
    const endpoints = [
      '/api/accounts',
    ...
    for (const ep of endpoints) {
      const res = await page.request.get(`${baseUrl}${ep}`, {
    ...
    if (!response) {
      test.info().annotations.push({
        type: 'note',
        description: 'No API endpoint returned status 200. Check available endpoints.',
      });
    ...
    test.info().annotations.push({
      type: 'note',
      description: `Accounts API endpoint used: ${usedEndpoint}`,
    });
    ...
    const accounts = Array.isArray(body)
      ? body
      : body?.data || body?.accounts || body?.items || [];

    expect(Array.isArray(accounts)).toBeTruthy();
    expect(accounts.length).toBeGreaterThan(0);

    // Verify required fields in first account
    const account = accounts[0] || {};
    const hasId = account.id || account._id;
    const hasName = account.name || account.accountName;
    const hasCode = account.code || account.accountCode;
    const hasType = account.type || account.accountType;
